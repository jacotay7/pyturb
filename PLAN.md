...
| **M4** (adoption) 🟡 | Phase 5 gallery ✅ + Phase 6 | **Mostly done** — `validation/validate.py` + `docs/validation.md` (5 theory checks, PASS/FAIL, figure); **mkdocs-material site** (index/quickstart/concepts/validation/comparison/interop/api, mkdocstrings API); **examples gallery** (`examples/01`–`05`); `py.typed` + version single-sourced from metadata; **CHANGELOG/CONTRIBUTING**; **CI** gains ruff-lint + coverage + strict docs-build; **`release.yml`** (PyPI trusted publishing on tag) and **`docs.yml`** (GH Pages) written; `pyturb.benchmark()`; version → **0.2.0**. *Needs a human:* tag `v0.2.0` to publish, enable GH Pages + the trusted publisher, add the animated-GIF asset. |
| **M5** (polish) 🟡 | | **`cho_factor` extruder robustness done** (Cholesky + lstsq fallback, shared by both extruders). Remaining extras: conda-forge, `optimal_grouping`/GCTM compression, dry/wet chromatic split. |
